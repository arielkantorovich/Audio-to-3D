    ...
    def prepare_embeddings(self):
        ...
        if self.opt.text is not None:
            ...
            if 'SD' in self.guidance:
                self.embeddings['SD']['default'] = self.guidance['SD'].get_text_embeds([self.opt.text], self.opt.audio, self.opt.placeholder_token)
                self.embeddings['SD']['uncond'] = self.guidance['SD'].get_text_embeds([self.opt.negative])

                for d in ['front', 'side', 'back']:
                    self.embeddings['SD'][d] = self.guidance['SD'].get_text_embeds([f"{self.opt.text}, {d} view"], self.opt.audio, self.opt.placeholder_token)
